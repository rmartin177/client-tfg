...
import './Modal.css'
const Modal = (props) => {
    const { listOfAuthors, showModal, setauthorsChoosen, authorsChoosen } = props;
    useEffect(() => {
        if (showModal !== false) {
    ...
    const selectedAuthors = (e) => {
        ...
        for (let index = 0; index < inputsChecked.length; index++) {
            let obj = {"author": inputsChecked[index].value, "link": inputsChecked[index].getAttribute("link")};
            
            const returnedTarget = authorsChoosen.concat(obj);
