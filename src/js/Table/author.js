...
export const searchOnTableAuthors = (json) => {
  ...
  for (
    let index = 0;
    index < json.authors.length && elm !== undefined;
    ++index
  ) {
...
export function writeAuthorsOnTable(
  json,
  paginaActualAutores,
  ...
  for (let index = inicio; index < nAutores; ++index) {
    auxAuthorsWrite(json, index, dataTableAuthors, filtersAuthors);
  }
}
...
function auxAuthorsWrite(json, index, dataTableAuthors, filtersAuthors) {
  ...
  if (elm !== undefined) {
    ...
    //--- Jcr ---
    addArrtoTable(elm.jcr, tableHeadAuthors, filtersAuthors["checkJRC"]);
    //--- Ggs ---
    addArrtoTable(elm.ggs, tableHeadAuthors, filtersAuthors["checkGGS"]);
    //--- Core ---
    addArrtoTable(elm.core, tableHeadAuthors, filtersAuthors["checkCore"]);
